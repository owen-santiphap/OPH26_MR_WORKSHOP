...
public:
	...
	/**
	 * Deprecated, use GenerateProceduralSceneMesh instead.
	...
	/**
	 * Generate the triangle mesh of the scene mesh anchor. For the generation successful the room needs
	 * to have an anchor that is labeled with GLOBAL_MESH.
	 * @param Material   Material to apply on the generated triangle mesh.
	 * @return           On Success true, otherwise false.
	 */
	UFUNCTION(BlueprintCallable, Category = "MR Utility Kit")
	bool GenerateProceduralSceneMesh(UMaterialInterface* Material);
...
public:
	...
	AMRUKAnchor* FindAnchorByUuid(const FOculusXRUUID& Uuid);

	void RemoveAnchor(AMRUKAnchor* Anchor);
